...
#   e   the instrumentation crate is absent from every contract's wasm
#       dependency closure (closure-gate.sh) — never by comparing hashes
#
...
echo
echo "== the instrumentation crate's closure =="
# A dependency's IDENTITY moves a contract's hash even when nothing calls it
# (F37), so this is the check that stops an accidental epoch. It is NOT done by
# comparing hashes: the input→hash map is not one-to-one, so a hash that
# happens to match after adding a dependency proves nothing about the closure.
checks=$((checks + 1))
if ./closure-gate.sh; then
  printf '\n  %-64s %s\n' "closure-gate.sh (its own checks, counted as one here)" ok
else
  printf '\n  %-64s %s\n' "closure-gate.sh (its own checks, counted as one here)" FAILED
  failures=$((failures + 1))
fi
